...
на базе платформ Arbnb и другие примеры;
...
Компания Apple в рамках со-конкуренции допустила  Kindle в свой сервис App Store, хотя имеет свой книжный сервис iBooks. Этот пример показывает, 
...
<b>Иными словами, меняется экономика компании-производителя</b>.  

подробнее см. блог Модель PaaS. 
...
В результате обработки этой информации появляются персонализированные предложения для потребителей: вместе с этим товаром ищут ...; вам поступает реклама товаров, которые вы ранее искали в сети; предложение зайти в магазин, мимо которого вы проходите.  
...
Многочисленные примеры показывают, что компании раскрывают свои API и приложения, а это позволяет клиентам и разработчикам 
...
Например, компания РБК оценивает инфляцию по потребительским товарам и по регионам на основе данных, накопленных платформой Avito. Доступ к информации по чекам сделает такие оценки более точными.
 ...
 Открытие доступа к большим данным приводит к проблеме извлечения знаний из этих данных (data mining - "раскопки" данных), к чему компании пока не готовы. 
